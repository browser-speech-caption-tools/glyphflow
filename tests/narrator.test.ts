...
describe("KaraokeNarrator", () => {
  ...
  it("marks captions so whitespace and newlines are preserved by the stylesheet", () => { const target = document.createElement("div"); new KaraokeNarratorImpl({ text: "  one  \ntwo", target, driver: new Driver() }); expect(target.textContent).toBe("  one  \ntwo"); expect(target.classList.contains("kn-caption")).toBe(true); });
  it("completes skipped words without learning a mixed duration", () => { const driver = new Driver(); const target = document.createElement("div"); const n = new KaraokeNarratorImpl({ text: "one two three", target, driver }); n.speak(); driver.handlers!.boundary({ charIndex: 0, elapsedTime: 0, name: "word" }); driver.handlers!.boundary({ charIndex: 8, elapsedTime: 0.8, name: "word" }); const words = target.querySelectorAll<HTMLElement>(".kn-word"); expect(words[0]?.style.getPropertyValue("--kn-progress")).toBe("100%"); expect(words[1]?.style.getPropertyValue("--kn-progress")).toBe("100%"); expect(n.getDiagnostics().samples).toHaveLength(0); driver.handlers!.end(); expect(words[2]?.style.getPropertyValue("--kn-progress")).toBe("100%"); });
