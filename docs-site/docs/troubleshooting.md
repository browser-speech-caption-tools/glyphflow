...
Please [open a GitHub issue](https://github.com/speech-caption-tools/glyphflow/issues/new?template=bug_report.yml) when behavior differs from this documentation. Before filing, check existing issues and this troubleshooting guide. Include reproducible steps, expected and actual behavior, GlyphFlow version, browser and OS versions, selected voice details, and relevant state changes or diagnostics. A minimal code example is especially helpful; remove private text and credentials first. See the [contribution guide](https://github.com/speech-caption-tools/glyphflow/blob/main/CONTRIBUTING.md) for the full reporting checklist.
