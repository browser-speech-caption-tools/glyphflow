...
function estimateUnits(word: string): number {
  ...
  return Math.max(1, groups * 2 + Math.max(0, normalized.length - groups) * 0.2 - digraphs * 0.15);
}
...
export function tokenize(text: string, locale = "en"): WordToken[] {
  ...
  if (typeof Intl !== "undefined" && "Segmenter" in Intl) {
    ...
    for (const part of segmenter.segment(text)) {
      if (part.isWordLike) positions.push({ text: part.segment, start: part.index, end: part.index + part.segment.length });
    }
  } else {
    for (const match of text.matchAll(/[\p{L}\p{N}]+(?:['’][\p{L}\p{N}]+)*/gu)) {
      positions.push({ text: match[0], start: match.index ?? 0, end: (match.index ?? 0) + match[0].length });
    }
  }
  return positions.filter((item) => isWord(item.text)).map((item, index) => ({
    ...item,
...
export function findTokenIndex(tokens: readonly WordToken[], charIndex: number): number {
  if (!tokens.length) return -1;
  const exact = tokens.findIndex((token) => charIndex >= token.start && charIndex < token.end);
