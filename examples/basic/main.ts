import { createKaraokeNarrator, getSpeechSynthesisSupport, getVoices, type KaraokeNarrator, type WordTimingSample } from "../../src/index";
import "../../src/styles.css";
import "./style.css";

const byId = <T extends HTMLElement>(id: string) => document.querySelector<T>(`#${id}`)!;
const text = byId<HTMLTextAreaElement>("text"), voice = byId<HTMLSelectElement>("voice"), rate = byId<HTMLInputElement>("rate"), rateValue = byId<HTMLOutputElement>("rate-value"), caption = byId<HTMLDivElement>("caption"), rows = byId<HTMLTableSectionElement>("timings"), status = byId<HTMLParagraphElement>("status");
let narrator: KaraokeNarrator | undefined; let latest: ReturnType<KaraokeNarrator["getDiagnostics"]> | undefined;
function populateVoices() { const voices = getVoices(); voice.replaceChildren(...(voices.length ? voices.map((item, index) => new Option(`${item.name} (${item.lang})`, String(index))) : [new Option("No voices available yet", "")])); }
...
byId<HTMLButtonElement>("speak").onclick = () => { narrator?.destroy(); rows.replaceChildren(); latest = undefined; const voices = getVoices(); narrator = createKaraokeNarrator({ text: text.value, target: caption, voice: voice.value ? voices[Number(voice.value)] : undefined, rate: Number(rate.value), onWordTiming: addRow, onStateChange: (state, detail) => { latest = detail?.diagnostics; status.textContent = detail?.reason ? `${state}: ${detail.reason}` : state; } }); narrator.speak(); };
