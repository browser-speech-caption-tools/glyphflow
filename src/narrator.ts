...
export class KaraokeNarratorImpl implements KaraokeNarrator {
  ...
  private onEnd(id: number): void { if (id !== this.session || this.destroyed) return; cancelAnimationFrame(this.raf); if (!this.boundaries) { this.setState("unsupported", "The selected voice emitted no word boundary events"); return; } for (let index = this.active; index < this.tokens.length; index++) this.renderer.complete(index); this.setState("ended"); }
